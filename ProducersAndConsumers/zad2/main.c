...
int write_mode;         // 0 deafult, 1 information about all consumers
int ns;                 // 0 read till EOF, >0 read during ns seconds

struct global_buf {
    sem_t sem;
    char** array;           // array od pointers to lines
...
void* consumer(void* arg){
    ...
    while (1){
        ...
        if (write_mode == 1)
            printf("Consumer %lu: trying to lock semaphore\n ",pthread_self());
        ...
        if (write_mode == 1){
            printf("Consumer: %lu unlocking semaphore\n", pthread_self() );
        }
        increase_sem();
    }
    exit(0);
...
void* producer(void* arg){
    ...
    while(!feof(file_ptr)){
        ...
        if (write_mode == 1)
            printf("Producer %lu: trying to lock semaphore\n ",pthread_self());

        decrease_sem();

        if (write_mode == 1)
            printf("Producer %lu: got semapathore\n", pthread_self());
        ...
        if (write_mode == 1)
            printf("Producent %lu: write line to array\n", pthread_self());
      ...
      //  printf("LINE: %s",  buf.array[buf.next_to_write]);
        buf.next_to_write++;
        ...
        if (write_mode == 1){
            printf("Producer: %lu unlocking semaphore\n", pthread_self() );
        }
        increase_sem();
    }
...
int main(int argc, char **argv){
    ...
    if (argc != 2){
        ...
        if (sem_init(&buf.sem, 0, 1) != 0) {
            perror("Cannot create samaphore");
            exit(-1);
        }
        ...
        // save values to arguments
        fscanf(file_ptr, "%d", &P);
        fscanf(file_ptr, "%d", &K);
        fscanf(file_ptr, "%d", &N);
        fscanf(file_ptr, "%s", input_name);
        fscanf(file_ptr, "%d", &L);
        fscanf(file_ptr, "%d", &search_mode);
        fscanf(file_ptr, "%d", &write_mode);
        fscanf(file_ptr, "%d", &ns);

        printf("%s\n", input_name);
        printf("P: %d, K: %d, N: %d, L: %d, search_mode: %d, write_mode: %d, ns: %d\n", P, K, N, L, search_mode, write_mode, ns);
